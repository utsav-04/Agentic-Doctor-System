...
def chat(req: ChatRequest):
    ...
    # ── Resuming: an order/booking tool paused for a patient name ────
    elif waiting_for == "patient_name":
        result = graph.invoke({
            **prev_state,
            "patient_name": req.message.strip().title(),
        })
    ...
    # ── Order/booking tool needs a patient name — pause and ask ──────
    if result.get("stage") == "need_patient_name":
        reply = "Who is this order/appointment for? (patient's name)"
        update_session(req.session_id, graph_state=result, waiting_for="patient_name")
        append_history(req.session_id, "assistant", reply)
        return ChatResponse(session_id=req.session_id, reply=reply, awaiting="patient_name")
