...
from src.tools.doctor_tool import doctor_finder
from src.tools.first_aid_tool import first_aid_guide
from src.integrations.order_client import call_order_tool
from src.logger import get_logger

logger = get_logger(__name__)

NEED_LOCATION = "NEED_LOCATION"
NEED_PATIENT_NAME = "NEED_PATIENT_NAME"
...
def _call_order_medicine(state: AgentState) -> str:
    patient_name = state.get("patient_name")
    if not patient_name:
        return NEED_PATIENT_NAME

    age = state.get("user_age")
    age_val = int(age) if age and str(age).isdigit() else None
    order_text = state.get("user_input", "").strip()

    result = call_order_tool(
        "place_medicine_order",
        patient_name=patient_name,
        medicine_name=order_text,
        quantity=1,
        patient_age=age_val,
        notes=f"Symptoms context: {state.get('symptoms', '')}",
    )
    return result.get("message", "Could not submit the medicine order.")


def _call_order_lab(state: AgentState) -> str:
    patient_name = state.get("patient_name")
    if not patient_name:
        return NEED_PATIENT_NAME
    ...
    result = call_order_tool(
        "place_lab_order",
        patient_name=patient_name,
        test_names=order_text,
        patient_age=age_val,
        notes=f"Symptoms context: {state.get('symptoms', '')}",
    )
    return result.get("message", "Could not submit the lab order.")


def _call_book_doctor(state: AgentState) -> str:
    patient_name = state.get("patient_name")
    if not patient_name:
        return NEED_PATIENT_NAME
    ...
    result = call_order_tool(
        ...
        patient_name=patient_name,
        doctor_name=order_text,
        hospital="",
        preferred_date="",
        patient_age=age_val,
        notes=f"Symptoms context: {state.get('symptoms', '')}",
    )
    return result.get("message", "Could not book the appointment.")
...
_TOOL_DISPATCH = {
    ...
    "first_aid": _call_first_aid,
    "doctor": _call_doctor,
    "order_medicine": _call_order_medicine,
    "order_lab": _call_order_lab,
    "book_doctor": _call_book_doctor,
}
...
def executor_node(state: AgentState) -> dict:
    ...
    for tool_name in plan:
        ...
        if output == NEED_PATIENT_NAME:
            logger.info("executor_node — '%s' needs a patient name, pausing", tool_name)
            messages.append(f"[Executor] Waiting for patient_name to run '{tool_name}'.")
            return {
                "tool_outputs": tool_outputs,
                "messages": messages,
                "stage": "need_patient_name",
            }
