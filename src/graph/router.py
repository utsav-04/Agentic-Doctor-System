...
VALID_TOOLS = {
    "medicine", "lab", "doctor", "first_aid",       # recommend / inform only
    "order_medicine", "order_lab", "book_doctor",   # actually place an order / booking
}
...
Available tools:
  medicine        -> user wants medicine SUGGESTIONS, drug info, or dosage advice
  lab             -> user wants lab test / diagnostic test SUGGESTIONS
  doctor          -> user wants to find/recommend a nearby doctor or hospital
  first_aid       -> user wants immediate first aid or home remedy guidance
  order_medicine  -> user wants to actually ORDER/BUY/PURCHASE a specific medicine
  order_lab       -> user wants to actually ORDER/BOOK a specific lab test
  book_doctor     -> user wants to actually BOOK/SCHEDULE an appointment with a doctor
...
Rules:
  ...
  - Include "doctor" only if the user asks to find one, or if first_aid /
    criticality context already implies urgent referral.
  - order_medicine / order_lab / book_doctor are DIFFERENT from
    medicine/lab/doctor — they mean the user wants an actual transaction
    to happen, not just information. Only use them when the message has a
    clear action verb: "order", "buy", "purchase", "place an order for",
    "book", "schedule". A plain symptom description or "what should I
    take" NEVER triggers these, even if medicine/lab/doctor also fire.
  - A query can need more than one tool when it genuinely does
    (e.g. "what medicine should I take and what tests should I get done"
    -> ["medicine", "lab"] — here the user explicitly asked for tests).
  - If the message is a greeting or general question with no medical
...
"what tests should I get for these symptoms and any medicine" -> ["medicine", "lab"]
"find me a doctor nearby" -> ["doctor"]
"order the paracetamol tablets for me" -> ["order_medicine"]
"please book the CBC test" -> ["order_lab"]
"book an appointment with Dr. Anand Iyer" -> ["book_doctor"]
"hi" -> []
"""
    response = llm.call(prompt)
    plan = _parse_plan(response)
