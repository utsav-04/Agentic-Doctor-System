...
class AgentState(TypedDict, total=False):
    ...
    delivered_tools:  List[str]     # tool names whose output has already been shown to the
                                     ...
                                     # otherwise every follow-up re-shows everything already
                                     # sent in an earlier chat message.
    patient_name:     str           # needed by order_medicine/order_lab/book_doctor —
                                     # current chat flow never collects this on its own,
                                     # so executor_node pauses (like the doctor-location
                                     # case) until it's supplied.
